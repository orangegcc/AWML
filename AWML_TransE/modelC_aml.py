...
def TrainFn1Member(fnsim, embeddings, density, marw, marb, leftop, rightop, marge=1.0, rel=True):
    ...
    # pdb.set_trace()
    marl = T.nnet.sigmoid(T.dot(marw.E, density.E) + marb.E)
    margl = S.dot(marl, inpo)
    marr = T.nnet.sigmoid(T.dot(marw.E, density.E) + marb.E)
    margr = S.dot(marr, inpo)

    costl, outl = margincost(simi, similn, marge*margl)
    costr, outr = margincost(simi, simirn, marge*margr)
    cost = costl + costr
    ...
    if rel:
        ...
        costol, outol = margincost(simi, simion, marge*margl)
        costor, outor = margincost(simi, simion, marge*margr)
        costo = (costol+costor)/2
        cost += costo
        out = T.concatenate([out, outol])
        list_in += [inpon]
    ...
    :output mean(out): ratio of examples for which the margin is violated,
                       i.e. for which an update occurs.
    """
    return theano.function(list_in, [T.mean(cost), T.mean(out), marl, marr],
...
def FilteredRankingScoreIdx_train(sl, sr, idxl, idxr, idxo, true_triples, subrel2rel):
	...
	oo = subrel2rel[o+40943]    #聚类前关系的序号
        ...
        io=np.argwhere(true_triples[10000:,1]==o).reshape(-1,)
        ir=np.argwhere(true_triples[:,2]==r).reshape(-1,)
	ioo=np.argwhere(true_triples[0:10000,1]==oo-40943).reshape(-1,)
