...
def WNexp(state, channel):
    ...
    outb = []
    state.bestvalid = -1
    #os.mkdir('./FB15k_TransC/margel/')
    #os.mkdir('./FB15k_TransC/marger/')
    ...
    for epoch_count in xrange(1, state.totepochs + 1):
        ...
        for i in range(state.nbatches):  #nbatches = 100
        ...
        if (epoch_count % 50) == 0:
            #f = open('./FB15k_TransC/margel/'+str(epoch_count)+'.pkl','w')
            #cPickle.dump(outtmp[2], f, -1)
            #f.close()
            #g = open('./FB15k_TransC/marger/'+str(epoch_count)+'.pkl','w')
