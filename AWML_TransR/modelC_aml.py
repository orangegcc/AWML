...
def TrainFn1Member(fnsim, embeddings, density, marw, marb, leftop, rightop, marge=1.0, rel=True):
    ...
    # pdb.set_trace()
    mar = T.nnet.sigmoid(T.dot(marw.E, density.E) + marb.E)
    marg = S.dot(mar, inpo)

    costl, outl = margincost(simi, similn, marge*marg)
    costr, outr = margincost(simi, simirn, marge*marg)
    cost = costl + costr
    ...
    if rel:
        ...
        costo, outo = margincost(simi, simion, marge*marg)
        cost += costo
