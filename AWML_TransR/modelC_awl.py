...
def RankRightFnIdx(fnsim, embeddings, leftop, rightop, subtensorspec=None):
    ...
    :param subtensorspec: only measure the similarity score for the entities
                          ...
                          entities).
    """
    embedding, relationV, relationM = parse_embeddings(embeddings)
    ...
    relv = (relationV.E[:, idxo]).reshape((1, relationV.D))  #rell与relr一样
    relm = (relationM.E[:, idxo]).reshape((1, relationM.D))
    tmp = leftop(lhs, relm)  #leftop返回二者之和
    simi = fnsim(tmp.reshape((1, tmp.shape[1]))+relv, rightop(rhs, relm)) #rightop返回前者
    """
    Theano function inputs.
    :input idxl: index value of the 'left' member.
    :input idxo: index value of the relation member.
...
def RankRightFnIdxvt(fnsim, embeddings, leftop, rightop, subtensorspec=None):
    """
    This function returns a Theano function to measure the similarity score of
    all 'right' entities given couples of relation and 'left' entities (as
    index values).
    ...
    lhs = (embedding.E[:, idxl]).reshape((1, embedding.D))
    rhs = (embedding.E[:, idxr]).reshape((1, embedding.D))
    rell = (relationl.E[:, idxo]).reshape((1, relationl.D))  #rell与relr一样
    relr = (relationr.E[:, idxo]).reshape((1, relationr.D))
    tmp = leftop(lhs, rell)  #leftop返回二者之和
    simi = fnsim(tmp.reshape((1, tmp.shape[1])), rightop(rhs, relr)) #rightop返回前者
    """
    Theano function inputs.
    :input idxl: index value of the 'left' member.
    :input idxo: index value of the relation member.
    ...
    :output simi: vector of score values.
    """
    return theano.function([idxl, idxo, idxr], [simi], on_unused_input='ignore')
...
def RankLeftFnIdx(fnsim, embeddings, leftop, rightop, subtensorspec=None):
    ...
    if subtensorspec is not None:
        ...
        lhs = embedding.E.T
    rhs = (embedding.E[:, idxr]).reshape((1, embedding.D))
    relv = (relationV.E[:, idxo]).reshape((1, relationV.D))
    relm = (relationM.E[:, idxo]).reshape((1, relationM.D))
    tmp = rightop(rhs, relm)
    simi = fnsim(leftop(lhs, relm)+relv, tmp.reshape((1, tmp.shape[1])))
    """
    Theano function inputs.
    :input idxr: index value of the 'right' member.
    :input idxo: index value of the relation member.
...
def RankLeftFnIdxvt(fnsim, embeddings, leftop, rightop, subtensorspec=None):
    """
    This function returns a Theano function to measure the similarity score of
    all 'left' entities given couples of relation and 'right' entities (as
    index values).
...
def RankRelFnIdx(fnsim, embeddings, leftop, rightop, subtensorspec=None):
    ...
    # We compute the score only for a subset of entities
    if subtensorspec is not None:
        relv = (relationV.E[:, :subtensorspec]).T
        relm = (relationM.E[:, :subtensorspec]).T
    else:
        relv = relationV.E.T
        relm = relationM.E.T
    # hack to prevent a broadcast problem with the Bilinear layer
    if hasattr(leftop, 'forwardrankrel'):
        tmpleft = leftop.forwardrankrel(lhs, relm)
    else:
        tmpleft = leftop(lhs, relm)
    if hasattr(rightop, 'forwardrankrel'):
        tmpright = rightop.forwardrankrel(rhs, relm)
    else:
        tmpright = rightop(rhs, relm)
    simi = fnsim(tmpleft+relv, tmpright)
...
def TrainFn1Member(fnsim, embeddings, density, marw, marb, leftop, rightop, marge=1.0, beta=24.5, rel=True):
    """
    This function returns a theano function to perform a training iteration,
    ...
    lhs = S.dot(embedding.E, inpl).T
    rhs = S.dot(embedding.E, inpr).T
    relm = S.dot(relationM.E, inpo).T
    relv = S.dot(relationV.E, inpo).T
    lhsn = S.dot(embedding.E, inpln).T
    rhsn = S.dot(embedding.E, inprn).T
    simi = fnsim(leftop(lhs, relm)+relv, rightop(rhs, relm))
    # Negative 'left' member
    similn = fnsim(leftop(lhsn, relm)+relv, rightop(rhs, relm))
    # Negative 'right' member
    simirn = fnsim(leftop(lhs, relm)+relv, rightop(rhsn, relm))
    # pdb.set_trace()
    mar = (T.nnet.sigmoid(T.dot(marw.E, density.E) + marb.E) + beta) / (2*beta+1)
    marg = S.dot(mar, inpo)

    costl, outl = margincost(simi*marg, similn*(1-marg), marge)
    costr, outr = margincost(simi*marg, simirn*(1-marg), marge)
    cost = costl + costr
    ...
    if rel:
        ...
        inpon = S.csr_matrix()
        relvn = S.dot(relationV.E, inpon).T
        relmn = S.dot(relationM.E, inpon).T
        simion = fnsim(leftop(lhs, relmn)+relvn, rightop(rhs, relmn))
        costo, outo = margincost(simi*marg, simion*(1-marg), marge)
        cost += costo
    ...
    gradients_embedding = T.grad(cost, marw.E)
    newE = marw.E - lrembeddings * gradients_embedding
    updates.update({marw.E: newE})
    gradients_embedding = T.grad(cost, marb.E)
    newE = marb.E - lrembeddings * gradients_embedding
    updates.update({marb.E: newE})

    if type(embeddings) == list:
        # If there are different embeddings for the relation member.
        gradients_embedding = T.grad(cost, relationV.E)
        newE = relationV.E - lrparams * gradients_embedding
        updates.update({relationV.E: newE})
        gradients_embedding = T.grad(cost, relationM.E)
        newE = relationM.E - lrparams * gradients_embedding
        updates.update({relationM.E: newE})
    """
    Theano function inputs.
    :input lrembeddings: learning rate for the embeddings.
    :input lrparams: learning rate for the parameters.
    ...
    :output mean(out): ratio of examples for which the margin is violated,
                       i.e. for which an update occurs.
    """
    return theano.function(list_in, [T.mean(cost), T.mean(out), mar],
...
def FilteredRankingScoreIdx(sl, sr, idxl, idxr, idxo, true_triples):
    ...
    errl = []
    errr = []
    for l, o, r in zip(idxl, idxo, idxr): #tuple
...
def FilteredRankingScoreIdx_train(sl, sr, idxl, idxr, idxo, true_triples, subrel2rel):
    ...
    for l, o, r in zip(idxl, idxo, idxr): #tuple  #此处的r是0~2500关系字典中的序号
        # pdb.set_trace()
        oo = subrel2rel[o+14951]    #聚类前关系的序号
        il=np.argwhere(true_triples[:,0]==l).reshape(-1,)
        io=np.argwhere(true_triples[109071:,1]==o).reshape(-1,)
        ir=np.argwhere(true_triples[:,2]==r).reshape(-1,)
        ioo=np.argwhere(true_triples[0:109071,1]==oo-14951).reshape(-1,)

        inter_l = [i for i in ir if i in io or i in ioo]   #计算头实体h的rank时，（r，t）在正例三元组集中曾出现的所有序号
        rmv_idx_l = [true_triples[i,0] for i in inter_l if true_triples[i,0] != l]  #计算头实体h的rank时，需要滤掉的头实体序号
        scores_l = (sl(r, o)[0]).flatten()
        scores_l[rmv_idx_l] = -np.inf
        errl += [np.argsort(np.argsort(-scores_l)).flatten()[l] + 1]

        inter_r = [i for i in il if i in io or i in ioo]
        rmv_idx_r = [true_triples[i,2] for i in inter_r if true_triples[i,2] != r]
        scores_r = (sr(l, o)[0]).flatten()
        scores_r[rmv_idx_r] = -np.inf
        errr += [np.argsort(np.argsort(-scores_r)).flatten()[r] + 1]
...
def FilteredRankingScoreIdx_test(sl, sr, idxl, idxr, idxo, true_triples, rel2subrel):
    ...
    for l, o, r in zip(idxl, idxo, idxr): #tuple   #此处的r是0~1344原始关系字典中的序号
        il=np.argwhere(true_triples[:,0]==l).reshape(-1,) #返回需要测试的三元组的头实体、关系、尾实体分别在所有正例三元组中的序号，得到il、io、ir
        ir=np.argwhere(true_triples[:,2]==r).reshape(-1,)
	subrel = rel2subrel[o+14951]
        j = 0
	for so in subrel:
	    io1 = np.argwhere(true_triples[109071:,1]==so-14951).reshape(-1,)
	    io2 = np.argwhere(true_triples[0:109071,1]==o).reshape(-1,)
	    io = np.concatenate((io1, io2), axis = 0)
	    inter_l = [i for i in ir if i in io]
	    rmv_idx_l = [true_triples[i,0] for i in inter_l if true_triples[i,0] != l]
	    scores_l = (sl(r, so-14951)[0]).flatten()
	    scores_l[rmv_idx_l] = -np.inf
	    ...
	    inter_r = [i for i in il if i in io]
            rmv_idx_r = [true_triples[i,2] for i in inter_r if true_triples[i,2] != r]
            scores_r = (sr(l, so-14951)[0]).flatten()
            scores_r[rmv_idx_r] = -np.inf
...
def FilteredRankingScoreRelIdx_test(so, idxl, idxr, idxo, true_triples, rel2subrel):
    ...
    for l, o, r in zip(idxl, idxo, idxr): #tuple   #此处的r是0~1344原始关系字典中的序号
        ...
        subrel = rel2subrel[o+14951]
        j = 0
        #io1 = np.argwhere(true_triples[109071:,1]==s-14951).reshape(-1,)
        #io2 = np.argwhere(true_triples[0:109071,1]==o).reshape(-1,)
        #io = np.concatenate((io1, io2), axis = 0)
	...
	for s in subrel:
            eo = np.argsort(np.argsort(-scores_o)).flatten()[s-14951] + 1
            if j == 0:
                erroro = eo
            else:
                if eo < erroro:
