...
def RankRightFnIdx(fnsim, embeddings, leftop, rightop, subtensorspec=None):
    ...
    tmp = leftop(lhs, relm)
    simi = fnsim(tmp.reshape((1, tmp.shape[1]))+relv, rightop(rhs, relm))
    """
    Theano function inputs.
    :input idxl: index value of the 'left' member.
    :input idxo: index value of the relation member.
...
def FilteredRankingScoreIdx(sl, sr, idxl, idxr, idxo, true_triples):
    ...
    :param idxr: list of 'right' indices.
    :param idxo: list of relation indices.
    :to remove the triples in train, valid and test dataset
    """
    errl = []
    errr = []
    for l, o, r in zip(idxl, idxo, idxr):
...
def FilteredRankingScoreIdx_train(sl, sr, idxl, idxr, idxo, true_triples, subrel2rel):
    ...
    for l, o, r in zip(idxl, idxo, idxr): #tuple  #此处的r是0~2500关系字典中的序号  
        ...
        il=np.argwhere(true_triples[:,0]==l).reshape(-1,) #返回需要测试的三元组的头实体、关系、尾实体>分别在所有正例三元组中的位置，得到il、io、ir
        io=np.argwhere(true_triples[109071:,1]==o).reshape(-1,)
        ir=np.argwhere(true_triples[:,2]==r).reshape(-1,)
        ioo=np.argwhere(true_triples[0:109071,1]==oo-14951).reshape(-1,)
        
        inter_l = [i for i in ir if i in io or i in ioo]   #计算头实体h的rank时，（r，t）在正例三元组>集中曾出现的所有序号
...
def FilteredRankingScoreRelIdx_test(so, idxl, idxr, idxo, true_triples, rel2subrel):
    ...
    for l, o, r in zip(idxl, idxo, idxr): #tuple   #此处的o是0~1344原始关系字典中的序号
        ...
        rmv_idx_o = [true_triples[i,1] for i in inter_o if (true_triples[i,1]+14951) not in subrel]
...
def FilteredRankingScoreRelIdx(so, idxl, idxr, idxo, true_triples):
    ...
    for l, o, r in zip(idxl, idxo, idxr):
        ...
        inter_o = [i for i in ir if i in il] #0-483141
        rmv_idx_o = [true_triples[i,1] for i in inter_o if true_triples[i,1] != o] #0-1344
        scores_o = (so(l, r)[0]).flatten()
        #pdb.set_trace()
        scores_o[rmv_idx_o] = -np.inf
...
def FilteredRankingScoreIdxvt(sl, sr, idxl, idxr, idxo, true_triples, mat, ent2i, idx2entity):
    """
    This function computes the rank list of the lhs and rhs, over a list of
    lhs, rhs and rel indexes.
    ...
    :param idxr: list of 'right' indices.
    :param idxo: list of relation indices.
    :param mat: cluster of muticluster-relation
    :param ent2i: changed dictionary
    :param idx2entity: unchanged dictionary
    """
    errl = []
    errr = []
    for l, o, r in zip(idxl, idxo, idxr):
	...
	if (o+14951) not in mat:
	    ...
	    #pdb.set_trace()
	    err = []
	    k = int(mat[o+14951][np.argmax(mat[o+14951]), 0]) + 1
	    for j in range(k):
        	scores_l = (sl(r, ent2i[idx2entity[o+14951]+str(j)]-14951)[0]).flatten()
        	scores_l[rmv_idx_l] = -np.inf
		err += [np.argsort(np.argsort(-scores_l)).flatten()[l] + 1]
            errl += [min(err)]
	    ...
	    k = int(mat[o+14951][np.argmax(mat[o+14951]), 0]) + 1
            for j in range(k):
                scores_r = (sr(l, ent2i[idx2entity[o+14951]+str(j)]-14951)[0]).flatten()
                scores_r[rmv_idx_r] = -np.inf
                err += [np.argsort(np.argsort(-scores_r)).flatten()[r] + 1]
            errr += [min(err)]
