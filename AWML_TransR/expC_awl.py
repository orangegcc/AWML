...
def FB15kexp(state, channel):
    ...
    # Positives
    trainl = load_file(state.datapath + state.dataset + '-train-inpl_RC.pkl')
    trainr = load_file(state.datapath + state.dataset + '-train-inpr_RC.pkl')
    traino = load_file(state.datapath + state.dataset + '-train-inpo_RC.pkl')
    if state.op == 'SE' or state.op == 'TransR':
    ...
    if not state.loadmodel:
        ...
        if state.op == 'TransR' and type(embeddings) is not list:
            relationVec = Embeddings(np.random, state.Nrel, state.ndim, 'relvec')
            relationMat = Embeddings(np.random, state.Nrel, state.ndim * state.nhid, 'relmat')
            embeddings = [embeddings, relationVec, relationMat]
    ...
    if state.pretrain == 'True':
        with open('./FB15k_embedding/CTransR/best_valid_model.pkl','rb') as f:
            embeddings = cPickle.load(f)
    ...
    if state.op == 'Bi':
        ...
        trainfunc = TrainFn1Member(simfn, embeddings, density, marw, marb, leftop, rightop,
                marge=state.marge, beta=state.beta, rel=True)
    ...
    out = []
    outb = []
    # os.mkdir('./FB15k_TransRC_ap_den2/margel/')
    # os.mkdir('./FB15k_TransRC_ap_den2/marger/')
    os.mkdir('./FB15k_TransRC_ap_den4/embedding/')
    state.bestvalid = -1
    ...
    for epoch_count in xrange(1, state.totepochs + 1):
        ...
        if (epoch_count % state.test_all) == 0:
            ...
            out = []
            outb = []
            resvalid = FilteredRankingScoreIdx_test(ranklfunc, rankrfunc,
                    validlidx, validridx, validoidx, true_triples, rel2subrel)
            state.valid = np.mean(resvalid[0] + resvalid[1])
            restrain = FilteredRankingScoreIdx_train(ranklfunc, rankrfunc,
                    trainlidx, trainridx, trainoidx, true_triples, subrel2rel)
            ...
            if state.bestvalid == -1 or state.valid < state.bestvalid:
                restest = FilteredRankingScoreIdx_test(ranklfunc, rankrfunc,
                        testlidx, testridx, testoidx, true_triples, rel2subrel)
        ...
        if (epoch_count % 50) == 0:
            p = open('./FB15k_TransRC_ap_den4/embedding/'+str(epoch_count)+'.pkl','w')
            cPickle.dump(embeddings, p, -1)
            p.close()

    '''test = FilteredRankingScoreIdx(ranklfunc, rankrfunc, idxtl, idxtr, idxto, true_triples)
...
def launch(datapath='data/', dataset='FB15k', Nent=17418, rhoE=1, rhoL=5,
        Nsyn=14951, Nrel=2467, loadmodel=False, loademb=False, op='Unstructured',
        simfn='Dot', ndim=50, nhid=50, marge=1., lremb=0.1, lrparam=1.,
        nbatches=100, totepochs=2000, test_all=1, neval=50, seed=123, pretrain='True', beta=24.5
