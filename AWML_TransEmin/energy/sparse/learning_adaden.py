...
def TrainFn1Member(fnsim, embeddings, density, marw, marb, leftop, rightop, rel=True,
    ...
    costl, outl = loss(simi, similn, margin=loss_margin*margl)
    costr, outr = loss(simi, simirn, margin=loss_margin*margr)
    ...
    if rel:
        ...
        costo, outo = loss(simi, simion, margin=loss_margin*margl)
        cost += costo
    ...
    embeds = [embedding.E] + [marw.E] + [marb.E] + ([relationr.E, relationl.E] if (type(embeddings) == list) else [])
    ...
    # In TransE etc. the rate for predicates' embeddings (that do not get normalized) is rate_params, not rate_embeddings
    rates_embeddings = [rate_embeddings, rate_embeddings, rate_embeddings, rate_params, rate_params] if len(embeds) > 1 else [rate_embeddings, rate_embeddings, rate_embeddings] # [rate_embeddings for i in range(len(embeds))]
    ...
    return theano.function(list_in, [T.mean(cost), T.mean(out), marl, marr], updates=updates, on_unused_input='ignore')
