...
int main(int argc, char** argv)
{
    ...
    FILE* f1 = fopen("../data/fb15k/original/entity2id.txt","r");
	FILE* f2 = fopen("../data/fb15k/original/relation2id.txt","r");
	int x;
    ...
    FILE* f_kb = fopen("/home/minelab/guochenchen/data_FB15k/freebase_mtr100_mte100-train.txt","r");
    ...
    FILE* f_ent_vec = fopen("../best_valid_model.txt","r");
   ...
   // FILE* f_ent_vec = fopen("../data/entity_vec_word2vec.txt","r");
    ...
    double sum = 0;
    int n=train_e1.size();
    FILE* g_sim = fopen(("./similarity/similarity"+string(argv[1])+".txt").c_str(),"w");
    for (int i=0; i<n; i++)
        for (int j=0; j<n; j++)
        if (i!=j)
        {
    ...
    FILE* g_pre = fopen(("./pre/pre"+string(argv[1])+".txt").c_str(),"w");
    for (int i=0; i<n; i++)
        fprintf(g_pre,"%lf\n",sum);
    fclose(g_pre);
    FILE* g_env = fopen(("./id2index/id2index"+string(argv[1])+".txt").c_str(),"w");
    for (int i=0; i<n; i++)
    {
        fprintf(g_env,"%d\n",train_index[i]);
    }
