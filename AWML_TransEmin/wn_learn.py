...
import energy.evaluation_wn as evaluation
...
def learn(state):
    ...
    trainfunc = learning.TrainFn1Member(simfn, embeddings, leftop, rightop, rel=True,
        ...
        if (state.test_all is not None) and ((epoch_count % state.test_all) == 0):
            ...
            if dataset.has_test is True:
                ...
                if state.ranking_score_right:
                    ...
                    # relation prediction
                    restest1 = evaluation.RankingScoreRelIdx(rankofunc, testlidx, testridx, testoidx)
                    for en in [1,3,10]:
                        test_summary = evaluation.ranking_summary_rel(restest1, n=en, tag='raw test')
                    if (state.filtered):
                        restest_filtered1 = evaluation.FilteredRankingScoreRelIdx(rankofunc, testlidx, testridx, testoidx, true_triples)
                        for en in [1,3,10]:
